...
export default class PerformerAttributeQuery {
  constructor(ids, slug, primaryGenres, otherGenres, taxonomies, parentTaxonomies) {
    ...
    this.ids = ids;
    this.slug = slug;
    this.primaryGenreSlugs = QueryParameterBuilder.buildGenreSlugs(primaryGenres);
    this.otherGenreSlugs = QueryParameterBuilder.buildGenreSlugs(otherGenres);
  ...
  buildQueryParameters() {
    return {
      ...
      'genres[primary].slug': this.primaryGenreSlugs,
      'genres.slug': this.otherGenreSlugs,
    }
  }
};
